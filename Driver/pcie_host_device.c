/*
 * Driver for PTP-Card(Keywie KW-800).
 */
...
/* for clear the ready */
#define	PEX_OMBCR	0x9b20
/* close pcie host the interrupt. */
#define	PEX_OMB_DATA	0x9ba4
...
static ssize_t host_read(struct file *filp, char __user *buf, size_t size, loff_t *f_pos)
{
	if (size != sizeof(s64)) {
		return -EINVAL;
	}
...
static long host_ioctl(struct file *filp, unsigned int cmd, unsigned long  arg)
{
...
ssize_t host_write(struct file *filp, const char __user *buf, size_t size, loff_t *f_pos)
{
	return 0;
}
...
static int host_board_init(struct pci_dev *pdev, int minor)
{
	...
	int retval = 0;
#ifdef SYNC_FROM_SYSTEM_TIME
/* add the fuction sync the PC system time to PTP Board time */
//------------------------------------------------------
	u32 hi;
	u32 lo;
	u32 remainder;
...
//------------------------------------------------------
	...
	/* CHANGED: auto create mknod pcietime0, pcietime1 ... */
	device_create(ptp_pci_class, NULL, MKDEV (PTP_PCI_MAJOR,minor), NULL, "%s%d", PCIE_NOD, minor);
#ifdef SYNC_FROM_SYSTEM_TIME
